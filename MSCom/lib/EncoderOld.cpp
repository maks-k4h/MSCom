...
void EncoderOld::putc(char ch, unsigned bits) {
    if (bits > 8) {
...
void EncoderOld::putint(int val, unsigned bits) {
    int is = sizeof(val) * 8;
    if (bits > is) {
        std::string message = "ENCODER: Cannot take "
                              + std::to_string(bits) + " bits"
...
void EncoderOld::reset(bool resBuff) {
    if (resBuff) buff_.erase(buff_.begin(), buff_.end());
    bitOutCount_ = 0;
    bitCount_ = 0;
}

bool EncoderOld::getc(char &ch, unsigned int bits) {
...
bool EncoderOld::getint(int &val, unsigned int bits) {
    int is = sizeof(val) * 8;
...
void EncoderOld::roundBitCount() {
    while (bitCount_ % 8) ++bitCount_;
}

void EncoderOld::roundOutBitCount() {

...
/*
 * TODO
 *  1) sortingPatterns: rewrite Bubble Sort to Merge Sort
 *  2) Move arrays and buffers to nave96 class if it's possible
 *  3) Evaluate effectiveness in standalone function
 * */
...
nave96::CBlock::Pattern::Pattern(nave96::CBlock::Pattern &&that)
        : Pattern() {
    this->swap(that);
}
...
void nave96::CBlock::compress() {
    ...
    /*
    std::cout << "Patterns found: "<<ptsI_ << std::endl;
    for (int i=0; i<ptsI_; ++i) {
        std::cout << "Pattern pos: " << pts_[i].pos_
            << ", used: " << pts_[i].t_
            << ", length: " << int(pts_[i].l_)
            << ", effectivity: " << pts_[i].effect_
            << " (" << pts_[i].effect_/8 << " bytes)"<< std::endl;
    }
    */
...
/*
int nave96::CBlock::hasPattern(int pos, int len) {

    int hash{0};
    for (int i = 0; i < len; ++i) hash = hash*HSN_N + data_[pos+i];


    return hasPattern(pos, len, hash);
}
 */
...
void nave96::CBlock::sortPatterns() {
    ...
    while (changed) {
        changed = false;
        for (int i = 0; i < (int) ptsI_ - 1; ++i) {
            if (pts_[i].effect_ < pts_[i + 1].effect_) {
                changed = true;
                pts_[i].swap(pts_[i + 1]);
            }
        }
    }
}
